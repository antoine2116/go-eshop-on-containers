...
import (
	...
	"github.com/antoine2116/go-eshop-on-containers/internal/services/catalogservice/internal/features/gettings_items_by_brand_id/v1/dtos"
	"github.com/gin-gonic/gin"
	"net/http"
)

type getItemsByBrandId struct {
	params.CatalogRouteParams
}

func NewGetItemsBrandIdEndpoint(params params.CatalogRouteParams) customGin.Endpoint {
	return &getItemsByBrandId{
		CatalogRouteParams: params,
	}
}

func (ep *getItemsByBrandId) MapEndpoint() {
	ep.CatalogGroup.GET("/items/brand/:brandId", ep.handler())
}

func (ep *getItemsByBrandId) handler() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx := c.Request.Context()

		request := &dtos.GetItemsByBrandIdRequestDto{}
		...
		c.JSON(http.StatusOK, &dtos.GetItemsByBrandIdResponseDto{Items: paginationResultDto})
	}
}
