...
import (
	...
	reflectionHelper "github.com/antoine2116/go-eshop-on-containers/internal/pkg/reflection/reflection_helper"
...
func mapStructs[TDes any, TSrc any](src reflect.Value, dest reflect.Value) {
	...
	for _, keys := range profile {
		...
		err := processValues[TDes, TSrc](sourceFiledValue, destinationField)
		if err != nil {
			return
		}
	}
...
func mapSlices[TDes any, TSrc any](src reflect.Value, dest reflect.Value) {
	...
	for i := 0; i < src.Len(); i++ {
		...
		err := processValues[TDes, TSrc](srcVal, destVal)
		if err != nil {
			return
		}
	}
...
func mapPointers[TDes any, TSrc any](src reflect.Value, dest reflect.Value) {
	...
	err := processValues[TDes, TSrc](src.Elem(), val)
	if err != nil {
		return
	}
...
func mapMaps[TDes any, TSrc any](src reflect.Value, dest reflect.Value) {
	...
	for destMapIter.Next() && srcMapIter.Next() {
		...
		err := processValues[TDes, TSrc](srcMapIter.Key(), destKey)
		if err != nil {
			return
		}
		err = processValues[TDes, TSrc](srcMapIter.Value(), destValue)
		if err != nil {
			return
		}
